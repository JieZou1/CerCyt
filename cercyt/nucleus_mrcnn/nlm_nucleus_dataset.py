...
ROOT_DIR = os.path.abspath("Y:/Cytology/NLM_Data")
cLEAN2_DIR = os.path.join(ROOT_DIR, 'cLEAN2')
INK_DIR = os.path.join(ROOT_DIR, 'ink')

# Directories for intermediate results
NUCLEUS_DIR = os.path.abspath("Y:/Users/Jie/CerCyt/BD-cytology-25/Nucleus")
NUCLEUS_PATCH_DIR = os.path.join(NUCLEUS_DIR, 'patches')
...
class NlmNucleusDataset(utils.Dataset):
    ...
    def load_nucleus_from_ndpi(self, dataset_dir, subset, save_croped_patch=False):
        ...
        for ndpi_file in ndpi_list:
            ...
            for y in range(0, height, 1024-128):
                for x in range(0, width, 1024-128):
                    image_id = "{}.{}.{}.{}".format(ndpi_file, y, x, 1024)
                    image_path = os.path.join(NUCLEUS_PATCH_DIR, image_id+".tif")
                    self.add_image(
                        "nucleus",
                        image_id=image_id,
                        path=image_path
                    )
                    if save_croped_patch:
                        position = (int(x), int(y))
                        image = ndpi_slide.read_region(position, 0, (1024, 1024))
...
def crop_patches(subset):
    cLEAN2_set = NlmNucleusDataset()
    cLEAN2_set.load_nucleus_from_ndpi(ROOT_DIR, subset, True)
